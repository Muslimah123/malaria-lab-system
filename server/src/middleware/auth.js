...
const requirePermission = (permission) => {
  return (req, res, next) => {
    ...
    // Allow supervisors and admins to access most resources
...
/**
 * Analytics access middleware - allows supervisors and admins
 */
const requireAnalyticsAccess = (req, res, next) => {
  ...
  // Allow supervisors and admins to access analytics
  if (['admin', 'supervisor'].includes(req.user.role)) {
    return next();
  }

  // Check if technician has specific permission
  if (req.user.role === 'technician' && req.user.permissions?.canViewAllTests) {
    return next();
  }

  return res.status(403).json({
    success: false,
    message: 'Access denied. Analytics access requires supervisor role or canViewAllTests permission.'
  });
};
