...
if (!validResourceTypes.includes(logData.resourceType)) {
  ...
  /**
   * ✅ ADDED: Get API call count for performance metrics
   */
  async getApiCallCount(timeWindowMs) {
    try {
      const startTime = new Date(Date.now() - timeWindowMs);
      const count = await AuditLog.countDocuments({
        action: { $regex: /^(GET|POST|PUT|DELETE)/i },
        timestamp: { $gte: startTime }
      });
      return count;
    } catch (error) {
      logger.error('Error getting API call count:', error);
      return 0;
    }
  }

  /**
   * ✅ ADDED: Get error count for performance metrics
   */
  async getErrorCount(timeWindowMs) {
    try {
      const startTime = new Date(Date.now() - timeWindowMs);
      const count = await AuditLog.countDocuments({
        level: 'ERROR',
      ...
      logger.error('Error getting error count:', error);
      return 0;
    }
  }

  /**
   * ✅ ADDED: Get recent errors for performance metrics
   */
  async getRecentErrors(limit = 10) {
    try {
      ...
      return errors.map(error => ({
        type: error.action || 'Unknown',
        timestamp: error.timestamp,
        details: {
          message: error.message || error.details || 'No details available',
  ...
  /**
   * ✅ ADDED: Get user interactions for performance metrics
   */
  async getUserInteractions(userId, timeWindowMs) {
    try {
      const startTime = new Date(Date.now() - timeWindowMs);
      const interactions = await AuditLog.find({
        userId: userId,
        action: { $regex: /^(PAGE_VIEW|BUTTON_CLICK|FORM_SUBMIT|NAVIGATION)/i },
        timestamp: { $gte: startTime }
      })
      .sort({ timestamp: -1 })
      .lean();
  ...
  /**
   * ✅ ADDED: Track real-time system metrics
   */
  async trackSystemMetrics() {
    try {
      const metrics = {
        ...
        activeUsers: 0, // Would need to track from auth system
        errorRate: 0, // Would need to calculate from recent logs
        responseTime: 0 // Would need to track from request middleware
      };
