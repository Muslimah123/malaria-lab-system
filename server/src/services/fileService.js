...
class FileService {
  ...
  async getImageUrl(filePath) {
    try {
      ...
      logger.debug(`getImageUrl: filePath=${filePath}, uploadDir=${this.uploadDir}`);
      const relativePath = path.relative(this.uploadDir, filePath);
      const result = `/uploads/${relativePath.replace(/\\/g, '/')}`;
      logger.debug(`getImageUrl: relativePath=${relativePath}, result=${result}`);
      return result;
    } catch (error) {
