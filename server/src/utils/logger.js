...
// Enhanced diagnosis logging with processing mode and features
logger.enhancedDiagnosis = (testId, processingMode, featuresUsed, details = {}) => {
  logger.info(`Enhanced Diagnosis: ${testId} - ${processingMode}`, {
...
// Flask API interaction logging
logger.flaskApi = (endpoint, method, status, duration, details = {}) => {
  const level = status >= 400 ? 'warn' : 'info';
  logger.log(level, `Flask API: ${method} ${endpoint} - ${status}`, {
    type: 'flask_api',
    endpoint,
    method,
    status,
